...
export default function CartDrawer() {
  ...
  const handleCheckout = () => {
    if (!user) {
      toast.warning("⚠️ Vui lòng đăng nhập trước khi thanh toán!");
      setOpen(false);
      navigate("/login");
      return;
    }
          ...
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Hãy tìm sản phẩm ưng ý và thêm vào giỏ hàng bạn nhé
              </p>
            ...
            cartItems.map((item) => (
              <div
                key={item.id}
                className="flex gap-3 items-start border rounded-lg p-2 hover:shadow-sm transition"
              >
                <img
                  src={item.image}
                  alt={item.name}
            ...
            <SheetFooter className="p-4">
              <div className="w-full">
                ...
                <Button
                  ...
                  onClick={handleCheckout} // ✅ Kiểm tra đăng nhập ở đây
                >
                  Thanh toán →
                </Button>
              </div>
