...
const parseAIResponse = (raw) => {
  ...
  let parsed = tryParse(raw) || extractJSON(raw);

  if (parsed?.output && typeof parsed.output === "string") {
    const inner = tryParse(parsed.output) || extractJSON(parsed.output);
...
export default function ChatBox() {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_CHAT_URL}`, {
      ...
      body: JSON.stringify({ message: sendText, user_id: user?.id || user?.user_id || null }),
    });
