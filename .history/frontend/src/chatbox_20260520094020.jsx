...
function OrderCard({ order }) {
  ...
  return (
    <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, overflow: "hidden", marginBottom: 8, fontSize: 13 }}>
      <div style={{ padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ background: st.bg, color: st.color, borderRadius: 20, padding: "2px 10px", fontSize: 11, fontWeight: 600 }}>
            {st.label}
          </span>
          {order.items?.length > 0 && (
            <button onClick={() => setExpanded((v) => !v)} style={{ background: "none", border: "none", cursor: "pointer", color: "#9ca3af", fontSize: 14, padding: 0 }}>
              {expanded ? "▲" : "▼"}
            </button>
          )}
        </div>
      ...
      {expanded && order.items?.length > 0 && (
        <div style={{ borderTop: "1px solid #f3f4f6", padding: "8px 14px", background: "#fafafa" }}>
          {order.items.map((item, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: i < order.items.length - 1 ? "1px solid #f3f4f6" : "none", fontSize: 12 }}>
...
const parseAIResponse = (raw) => {
  ...
  if (parsed) {
    if (parsed.type === "order")
      return { type: "order", text: parsed.message || "", products: [], orders: parsed.orders || [] };
    if (parsed.type === "product")
      return { type: "product", text: parsed.message || "", products: parsed.products || [], orders: [] };
    if (parsed.type === "text")
      return { type: "text", text: parsed.message || raw, products: [], orders: [] };
    if (parsed.type === "cart")
      return { type: "cart", text: parsed.message || "", product: parsed.product, products: [], orders: [] };
    if (parsed.type === "payment")
      return { type: "payment", text: parsed.message || "", url: parsed.url, products: [], orders: [] };
...
export default function ChatBox() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const { type, text, products, orders, product, url } = parseAIResponse(raw);
      ...
      setMessages((prev) => [...prev, { sender: "bot", text: "❌ Lỗi kết nối tới server!", products: [], orders: [] }]);
    } finally {
      setLoading(false);
    }
  };
      ...
      {isOpen ? (
        <div className="chatbox-container animate-fade-in">
          ...
          <div className="chatbox-body">
                ...
                {msg.text ? (
                  <div className={`inline-block px-3 py-2 rounded-lg ${msg.sender === "user" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-900"}`}>
                    {msg.text.split(/(https?:\/\/[^\s]+)/g).map((part, i) =>
                      /^https?:\/\//.test(part) ? (
                        <a key={i} href={part} className="underline text-blue-500 break-all">{part}</a>
                      ) : part
                    )}
                  </div>
                ) : null}
                      ...
                      <div key={p.id || i} className="flex gap-3 border rounded-lg p-3 bg-white shadow">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <a href={`/laptop/${p.slug || p.id}`} rel="noreferrer" className="text-xs text-blue-500 underline">Xem chi tiết</a>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            {loading && <div className="text-left text-gray-400 italic text-sm">Bi đang trả lời...</div>}
          ...
          <div className="p-3 border-t flex gap-2">
            ...
            />
            <button onClick={handleSend} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">Gửi</button>
          </div>
        </div>
      ) : (
        <button onClick={() => setIsOpen(true)} className="p-4 rounded-full bg-blue-600 text-white shadow-lg hover:bg-blue-700 transition-all animate-glow animate-wiggle">
          <Bot size={32} />
        </button>
      )}
    </div>
