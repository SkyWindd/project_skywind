...
const parseAIResponse = (raw) => {
  ...
  if (parsed) {
    if (parsed.type === "order") return { type: "order", text: parsed.message || "", products: [], orders: parsed.orders || [] };
    if (parsed.type === "product") return { type: "product", text: parsed.message || "", products: parsed.products || [], orders: [] };
    if (parsed.type === "text") return { type: "text", text: parsed.message || raw, products: [], orders: [] };
    if (parsed.type === "cart") return { type: "cart", text: parsed.message || "", product: parsed.product, products: [], orders: [] };
    if (parsed.type === "payment") return { type: "payment", text: parsed.message || "", url: parsed.url, products: [], orders: [] };
  ...
  if (matchOrder) { const p = tryParse(matchOrder[0]); if (p) return { type: "order", text: p.message || "", products: [], orders: p.orders || [] }; }

  const matchProduct = raw.match(/\{[\s\S]*?"type"\s*:\s*"product"[\s\S]*?\}/);
  if (matchProduct) { const p = tryParse(matchProduct[0]); if (p) return { type: "product", text: p.message || "", products: p.products || [], orders: [] }; }

   const matchCart = raw.match(/\{[\s\S]*?"type"\s*:\s*"cart"[\s\S]*?\}/);
  if (matchCart) { const p = tryParse(matchCart[0]); if (p) return { type: "cart", text: p.message || "", product: p.product, products: [], orders: [] }; }

  const matchPayment = raw.match(/\{[\s\S]*?"type"\s*:\s*"payment"[\s\S]*?\}/);
  if (matchPayment) { const p = tryParse(matchPayment[0]); if (p) return { type: "payment", text: p.message || "", url: p.url, products: [], orders: [] }; }
      ...
      if (type === "cart" && product?.id) {
        try {
          await fetch(`${import.meta.env.VITE_API_URL}/cart`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ product_id: product.id, quantity: 1, user_id: user?.id || user?.user_id || null }),
          });
        } catch (cartErr) {
          console.error("Add to cart error:", cartErr);
        }
      }
